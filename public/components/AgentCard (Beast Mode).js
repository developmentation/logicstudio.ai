...
export default {
          ...
          <div class="mt-4 space-y-4">
            ...
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-2">
                <button 
                  ...
                  @click="triggerAgent"
                  :disabled="localCardData.data.status === 'inProgress'"
                >
                  Trigger
                </button>
            ...
            <div class="space-y-2">
              <label class="flex items-center gap-2">
                <input 
                  type="checkbox" 
                  v-model="localCardData.data.beastMode.enabled" 
                  class="form-checkbox" 
                />
                <span class="text-xs text-gray-400">Beast Mode</span>
              </label>

              <!-- Beast Mode Options -->
              <div v-if="localCardData.data.beastMode.enabled" class="space-y-2">
                <!-- Input Socket Selection -->
                <select
                  v-model="localCardData.data.beastMode.selectedInput"
                  class="w-full bg-gray-900 text-xs text-gray-200 px-2 py-1 rounded cursor-pointer"
                  style="border: 1px solid #374151;"
                  @mousedown.stop
                  @change="handleCardUpdate"
                >
                  <option value="">Select input socket...</option>
                  <option 
                    v-for="socket in localCardData.data.sockets.inputs" 
                    :key="socket.id" 
                    :value="socket.id"
                  >
                    {{socket.name}}
                ...
                <div class="space-y-2">
                  ...
                  <div class="space-y-1">
                    <div 
                      v-for="(cardId, index) in localCardData.data.beastMode.selectedCards" 
                      :key="index"
                      class="flex items-center justify-between gap-2 p-1 bg-gray-800 rounded"
                    >
                      <select
                        v-model="localCardData.data.beastMode.selectedCards[index]"
                        class="flex-1 bg-gray-900 text-xs text-gray-200 px-2 py-1 rounded cursor-pointer"
                        style="border: 1px solid #374151;"
                        @mousedown.stop
                        @change="handleCardUpdate"
                      ...
                      <button 
                        @click="removeBeastCard(index)"
                        class="text-gray-400 hover:text-gray-200 flex-shrink-0"
                      >
                        ×
                      </button>
                    </div>
  ...
  setup(props, { emit }) {
    ...
    const beastModeProcessing = Vue.ref(false);
    ...
    const activeCards = Vue.computed(() => props.activeCards);
    ...
    const addBeastCard = () => {
      ...
      localCardData.value.data.beastMode.selectedCards.push(null);
      handleCardUpdate();
    };

    const removeBeastCard = (index) => {
      localCardData.value.data.beastMode.selectedCards.splice(index, 1);
      handleCardUpdate();
    };

    const getCardName = (cardId) => {
      const card = props.activeCards.find(c => c.uuid === cardId);
      return card ? card.ui.name : null;
    };

// Updated processBeastMode function

// In AgentCard.js
// In AgentCard.js, update processBeastMode function
const processBeastMode = async () => {
  if (beastModeProcessing.value) {
    console.log('Beast Mode already processing');
    return;
  }
  
  const { selectedInput, selectedCards } = localCardData.value.data.beastMode;
  if (!selectedInput || !selectedCards.length) {
    console.log('Missing required Beast Mode values');
    return;
  }

  beastModeProcessing.value = true;
  localCardData.value.data.status = 'inProgress'; // Start with inProgress
  localCardData.value.data.beastMode.beastMemory = [];
  ...
  try {
    for (const cardId of selectedCards) {
      ...
      const card = props.activeCards.find(c => c.uuid === cardId);
      if (!card || !card.data.sockets.outputs?.length) continue;

      for (const outputSocket of card.data.sockets.outputs) {
        // Signal that we're processing
        localCardData.value.data.status = 'inProgress';
        handleCardUpdate();

        // Create connection and wait for establishment
        emit('connection-create', {
          sourceCardId: cardId,
          sourceSocketId: outputSocket.id,
          targetCardId: localCardData.value.uuid,
          targetSocketId: selectedInput
        });

        await Vue.nextTick();
        await new Promise(resolve => setTimeout(resolve, 250));

        // Set up completion watcher before triggering
        const processPromise = new Promise((resolve, reject) => {
          const unwatch = Vue.watch(
            () => localCardData.value.data.status,
            (newStatus) => {
              if (newStatus === 'complete' || newStatus === 'complete-interim' || newStatus === 'error') {
                unwatch();
                if (newStatus === 'complete' || newStatus === 'complete-interim') {
                  resolve();
                } else {
        ...
        // Store output in beast memory
        if (localCardData.value.data.sockets.outputs[0].value) {
          localCardData.value.data.beastMode.beastMemory.push(
            localCardData.value.data.sockets.outputs[0].value
          );
        }

        // Clean up connection
        emit('connection-remove-by-source-target', {
          sourceCardId: cardId,
          sourceSocketId: outputSocket.id,
          targetCardId: localCardData.value.uuid,
          targetSocketId: selectedInput
        ...
        // No need to set any special beast mode status, completedMessage watcher
        // will handle setting complete-interim or complete
      }
    }

    // Set final output and complete status
    if (localCardData.value.data.beastMode.beastMemory.length > 0) {
      const combinedOutput = localCardData.value.data.beastMode.beastMemory.join('\n\n');
      localCardData.value.data.output = combinedOutput;
      localCardData.value.data.sockets.outputs[0].value = combinedOutput;
      localCardData.value.data.status = 'complete';
      handleCardUpdate();
    }
  } catch (error) {
    console.error('Beast Mode processing error:', error);
    localCardData.value.data.status = 'error';
    handleCardUpdate();
  } finally {
    beastModeProcessing.value = false;
  }
};



// Updated triggerAgent function
const triggerAgent = async () => {
  ...
  // Beast Mode check moved to start of function
  if (localCardData.value.data.beastMode.enabled &&
      localCardData.value.data.beastMode.selectedInput &&
      localCardData.value.data.beastMode.selectedCards.some(id => id)) {
    await processBeastMode();
    return;
  }
  ...
  sendToServer(
    ...
    Vue.watch(sessionStatus, (newValue) => {
      // Only protect complete-interim status during beast mode
      if (localCardData.value.data.beastMode?.enabled && 
          localCardData.value.data.status === 'complete-interim') {
        return;
      }
...
// In AgentCard.js, add this helper function
 
// Update the completedMessage watcher
Vue.watch(completedMessage, (newValue) => {
  if (newValue) {
    localCardData.value.data.output = newValue;
    localCardData.value.data.sockets.outputs[0].value = newValue;
    
    // If in Beast Mode and not finished with all sockets
    if (localCardData.value.data.beastMode.enabled) {
      const totalSocketCount = localCardData.value.data.beastMode.selectedCards.reduce((total, cardId) => {
        const card = props.activeCards.find(c => c.uuid === cardId);
        return total + (card?.data?.sockets?.outputs?.length || 0);
      }, 0);
      
      if (localCardData.value.data.beastMode.beastMemory.length < totalSocketCount) {
        localCardData.value.data.status = 'complete-interim';
      } else {
        localCardData.value.data.status = 'complete';
      }
