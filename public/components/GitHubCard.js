 ...
 setup(props, { emit }) {
    ...
    const initializeCardData = (data) => {
        ...
        if (data.sockets?.outputs?.length) {
          baseData.sockets.outputs = data.sockets.outputs.map((socket, index) => ({
            ...createSocket({
              type: 'output',
              index,
...
const getAllChildrenStatus = (node) => {
    ...
    const loadSelectedFiles = async (selectedPaths) => {
        ...
        try {
            ...
            if (filesToLoad.length) {
                ...
                if (loadResults?.successful) {
                    loadResults.successful.forEach(file => {
                        const node = findNodeByPath(localCardData.value.treeData, file.path);
                        if (node) {
            ...
            const allSelectedPaths = Object.keys(localCardData.value.selectedNodes)
                .filter(key => {
            ...
            const newSockets = allSelectedPaths.map((path, index) => {
                ...
                const loadedFile = filesToLoad.includes(path) ? 
                    loadResults?.successful?.find(f => f.path === path) : null;
            ...
            if (affectedSockets.length) {
                emit('sockets-updated', createSocketUpdateEvent({
                    cardId: localCardData.value.uuid,
                    oldSockets,
                    newSockets: oldSockets.filter(s => !filesToLoad.includes(s.path)),
                    reindexMap: new Map(),
  ...
  const loadRepository = async () => {
    ...
    Vue.watch(() => props.cardData, (newData, oldData) => {
        ...
        if (newData.treeData && JSON.stringify(newData.treeData) !== JSON.stringify(oldData?.treeData)) {
